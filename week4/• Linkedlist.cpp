...
NODE *createNode(int data)
{
    ...
    node->key = data;
    node->p_next = nullptr;
    return node; // Placeholder return statement
}
...
void removeAfter(List *&L, int val)
{
    ...
    NODE* cur = L-> p_head;
    while(cur != NULL && cur -> p_next != NULL){
        if (cur -> key == val ){
            NODE* tmp = cur ->p_next;
...
void removePos(List *&L,int data, int pos) 
{
    ...
    if (pos == 0) {
        temp = L->p_head;
        L->p_head = L->p_head->p_next;
        if (!L->p_head) L->p_tail = NULL;
        delete temp;
        return;
    }
    NODE* current = L->p_head;
    int index = 0;
    while (current->p_next && index < pos - 1) {
        current = current->p_next;
        index++;
    }
    if (!current->p_next) return; 
    temp = current->p_next;
    current->p_next = temp->p_next;
    if (!current->p_next) L->p_tail = current; 
    delete temp;
}
    ...
    while (curr != NULL) {
        ...
        while (runner->p_next != NULL) {
            if (runner->p_next->key == curr->key) {
                NODE* dup = runner->p_next;
                runner->p_next = dup->p_next;
                if (dup == L->p_tail) {
                    L->p_tail = runner;
                }
                delete dup;
...
bool removeElement(List* &L, int key) 
{
    ...
    while (curr != NULL && curr->p_next != NULL) {
        if (curr->p_next->key == key) {
            NODE* tmp = curr->p_next;
            curr->p_next = tmp->p_next;
            if (tmp == L->p_tail) {
                L->p_tail = curr;
            }
            delete tmp;
